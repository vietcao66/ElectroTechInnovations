...
const CLIENT_BASE_URL = "http://localhost:3500";
...
const Inventory = () => {
  ...
  const handleUpdateProduct = async (values) => {
    ...
    try {
      ...
      const currentProduct = await productApi.getDetailProduct(id);
      const currentQuantity = currentProduct.product.quantity || 0;

      // Calculate new quantity by adding the input quantity
      ...
      const inventory = {
        ...
        // Add total quantity to update product's stock
      ...
      return axiosClient.put("/product/" + id, inventory).then((response) => {
        if (response === undefined) {
          ...
          notification["success"]({
            message: `Thông báo`,
            description: `Nhập hàng thành công. Số lượng nhập: ${newQuantity}`,
          });
      ...
      notification["error"]({
        message: `Thông báo`,
        description: "Có lỗi xảy ra khi nhập hàng",
      });
      setLoading(false);
    }
  };
  ...
  const handleProductList = async () => {
    try {
      await productApi
        ...
        .then((res) => {
          //   console.log(res);
          console.log("Dữ liệu sản phẩm:", res.data.docs); // Kiểm tra dữ liệu
    ...
    (async () => {
      try {
        ...
        form2.setFieldsValue({
          currentQuantity: response.product.quantity || 0, // Display current quantity
        });
  ...
  const handleViewDetails = (productId) => {
    window.open(`${CLIENT_BASE_URL}/product-detail/${productId}`, "_blank");
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      //   dataIndex: "inventory",
      key: "inventory",
      //   render: (res) => <span>{res?.valuesquantity}</span>,
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          {(record.quantity * record.price)?.toLocaleString("vi", {
            style: "currency",
            currency: "VND",
          })}
        </span>
      ...
      <Spin spinning={loading}>
        ...
        >
          <Form
            ...
            scrollToFirstError
          >
            <Form.Item name="currentQuantity" label="Tồn kho hiện tại">
              <Input disabled />
            </Form.Item>
            <Form.Item
              name="quantity"
